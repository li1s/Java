package Лебедев.Сергей;
/**
 * 
 * @author L1is
 * Учимся программировать на JAVA
 * Пишем простую игру Mines (Сапёр)
 * 
 */
				...
				if (!bangMine && !youWon) {
					if (e.getButton() == MOUSE_BUTTON_LEFT) // left button mouse
						if (field[y][x].isNotOpen()) {
							openCells(x, y);
							youWon = countOpenedCells == FIELD_SIZE * FIELD_SIZE - NUMBER_OF_MINES; // winning
																									// check
							if (bangMine) {
								bangX = x;
								bangY = y;
					...
					if (e.getButton() == MOUSE_BUTTON_RIGHT)
						field[y][x].inverseFlag(); // right button mouse
					if (bangMine || youWon)
